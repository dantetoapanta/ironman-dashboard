...
function navClass({ isActive }) {
  return `flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
    isActive ? "bg-surface-3 text-white" : "text-gray-400 hover:text-white hover:bg-surface-2"
  }`;
}
    ...
    <div className="min-h-svh flex flex-col bg-surface">
      <header className="no-print sticky top-0 z-20 border-b border-border bg-surface/95 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <div>
            <div className="text-sm font-bold tracking-tight text-white">70.3 FLORIDA BUILD</div>
      ...
      <nav className="no-print md:hidden fixed bottom-0 inset-x-0 z-20 flex border-t border-border bg-surface-2/95 backdrop-blur">
        {NAV_ITEMS.map((item) => (
