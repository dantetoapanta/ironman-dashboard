  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-wider text-gray-500">Week {week.weekNumber} of 21 · {week.phase.name}</div>
          <h1 className="text-xl font-bold text-white">{DAY_NAMES[new Date().getDay()]}, {new Date().toLocaleDateString("en-US", { month: "long", day: "numeric" })}</h1>
          {week.focus && <p className="text-sm text-gray-400 mt-1 max-w-xl">{week.focus}</p>}
        </div>
        <div className="flex items-center gap-3 bg-surface-2 border border-border rounded-xl px-4 py-2">
